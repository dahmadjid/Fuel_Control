...
brand_model_list = [{"brand": "Seat", "models": ["Alhambra", "Altea", "Altea XL", "Arosa", "Cordoba", "Cordoba Vario", "Exeo", "Ibiza", "Ibiza ST", "Exeo ST", "Leon", "Leon ST", "Inca", "Mii", "Toledo"]},
...
def randomVehicleIds(count):
    id_list = []
    while(len(id_list) != count):
        id = str(random.randint(99,2999)).zfill(5)+" 3"+str(random.randint(10,21))+" "+str(random.randint(1,48)).zfill(2)
        if id in id_list:
            print("damn")
            continue
        id_list.append(id)
    return id_list
def randomBrands(count):
    car_list = []
    while(len(car_list) != count):
        brand_index = random.randint(0,len(brand_model_list)-1)
        car = brand_model_list[brand_index]["brand"] +" "+ brand_model_list[brand_index]["models"][random.randint(0,len(brand_model_list[brand_index]["models"])-1)]
        if car in car_list:
            continue
        car_list.append(car)
    return car_list
...
#print(dictDataToRowData(dict["Base Camp 0"]))
