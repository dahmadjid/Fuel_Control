...
void PCD_Version(){
	...
	if (ver == 0x92)
	{
		ESP_LOGI("RC522_2","MFRC522 Version 2 detected.");
	}
	else if (ver == 0x91)
	{
		ESP_LOGI("RC522_2","MFRC522 Version 1 detected.");
	}
	else{
		ESP_LOGE("RC522_2","version error %d", ver);
        vTaskDelay(1000/portTICK_PERIOD_MS);
		ESP_LOGE("RC522_2","Restarting Now.");
		fflush(stdout);
		esp_restart();
	}
}
void PCD_WriteRegister(uint8_t Register , uint8_t value){
	...
	ret = spi_device_transmit(spi_handle,&t1);
...
void PCD_Init(){
	...
	ESP_LOGI("RC522_2","Initialization successful.");
...
void PCD_AntennaOn(){
	...
	ESP_LOGI("RC522_2","Antenna turned on.");
}
...
bool PICC_ReadCardSerial() {
	...
	//ESP_LOGI("RC522_2","PICC_ReadCardSerial: Status %s",GetStatusCodeName(result));
